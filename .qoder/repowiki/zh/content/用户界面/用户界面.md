...
## 更新摘要
**变更内容**   
- 集成 Tailwind CSS v4.3 现代化样式系统，替代传统 CSS 架构
- 引入设计令牌（Design Tokens）系统，支持深色/浅色模式完整调色板
- 更新 Vite 配置以支持 Tailwind CSS v4 插件和 Lightning CSS 编译
- 增强测试环境 polyfill 支持，完善拖拽事件和媒体查询模拟
- 实现原子类与组件样式的职责分离策略
...
**更新** 项目现已集成 Tailwind CSS v4.3 现代化样式系统，采用设计令牌驱动的主题架构，提供完整的深色/浅色模式支持和响应式设计能力。

文档同时覆盖组件使用示例、属性配置、事件处理、插槽用法、组件间通信与数据流向、Pinia 集成方式、样式定制与主题覆盖、自定义主题开发建议、性能优化与可访问性支持。

## 项目结构
界面层采用"顶部公共栏 + 左中右三栏"的布局模式，结合 Tailwind CSS v4 原子类与 Naive UI 组件库：
...
J --> K["main.css<br/>Tailwind CSS v4 设计令牌"]
K --> L["@theme 变量<br/>深色/浅色模式"]
```
...
- [ArchivePanel.vue:1-35](file://src/components/archive-panel/ArchivePanel.vue#L1-L35)
...
- [theme.ts:1-25](file://src/styles/theme.ts#L1-L25)
- [main.css:1-99](file://src/styles/main.css#L1-L99)
...
**更新** 所有组件现已采用 Tailwind CSS v4 原子类进行样式定义，实现了更好的可维护性和一致性。
...
## 架构总览
界面遵循"组件 → 组合式函数 → Pinia 状态"的分层组织，并结合 Tailwind CSS v4 的设计令牌系统：
- 组件负责视图与交互，使用原子类进行样式定义
- 组合式函数封装业务逻辑与响应式数据
- Pinia store 提供跨模块共享的状态（主题、面板宽度、插件禁用列表）
- Tailwind CSS v4 提供设计令牌驱动的样式系统
...
participant T as "main.css(Tailwind)"
U->>P : 点击"主题切换"按钮
P->>S : 调用 toggleTheme()
S-->>A : isDarkTheme 变化
A->>A : 计算 theme(dark/light)
A->>L : NConfigProvider 主题更新
L->>L : 设置 data-theme 属性
L->>T : CSS 变量自动切换
T-->>U : 界面主题切换完成
```
...
- [app.ts:1-57](file://src/stores/app.ts#L1-L57)
- [App.vue:1-24](file://src/App.vue#L1-L24)
- [AppLayout.vue:142](file://src/layout/AppLayout.vue#L142)
- [main.css:45-55](file://src/styles/main.css#L45-L55)

## 详细组件分析

### Tailwind CSS v4 集成与设计令牌系统
**新增** 项目现已全面集成 Tailwind CSS v4.3，采用现代化的设计令牌架构：

#### 设计令牌定义
- **颜色令牌**：`--color-primary`、`--color-bg-base`、`--color-text-primary` 等
- **间距令牌**：`--spacing-header`、`--spacing-statusbar`、`--spacing-left-panel` 等
- **圆角令牌**：`--radius-panel`、`--radius-card`、`--radius-button` 等

#### 主题模式支持
- **深色模式**：默认主题，使用 `#18181c` 基础背景色
- **浅色模式**：通过 `[data-theme='light']` 选择器覆盖令牌值
- **动态主题色**：支持蓝色、绿色、紫色、橙色四种主题色方案

#### Vite 配置更新
- 集成 `@tailwindcss/vite` 插件
- 使用 `@import "tailwindcss"` 指令替代传统 `@tailwind` 语法
- 启用 Lightning CSS 编译器以获得最佳性能

```mermaid
flowchart TD
Start(["初始化"]) --> LoadCSS["加载 main.css"]
LoadCSS --> DefineTokens["定义 @theme 设计令牌"]
DefineTokens --> ImportTailwind["导入 tailwindcss"]
ImportTailwind --> ThemeDetection{"检测 data-theme"}
ThemeDetection --> |dark| DarkMode["应用深色模式令牌"]
ThemeDetection --> |light| LightMode["应用浅色模式令牌"]
DarkMode --> ApplyStyles["应用 Tailwind 原子类"]
LightMode --> ApplyStyles
...
### 三栏布局与响应式调整
- 布局实现
  - 使用 Tailwind CSS v4 原子类构建网格布局：`grid grid-rows-[var(--spacing-header)_1fr_var(--spacing-statusbar)]`
  - 左右侧栏宽度由 Pinia 的 leftPanelWidth/rightPanelWidth 控制，支持折叠与最小/最大宽度约束。
  - 响应式间距通过 CSS 变量实现：`h-header`、`h-statusbar` 等
- 响应式策略
...
- 状态同步
  ...
  - 通过 `data-theme` 属性切换 CSS 变量，实现无缝主题切换。
...
UpdateStore --> ReRender["Tailwind 原子类重新渲染"]
...
**图示来源**
- [AppLayout.vue:142-286](file://src/layout/AppLayout.vue#L142-L286)
- [app.ts:1-57](file://src/stores/app.ts#L1-L57)
...
- [ArchivePanel.vue:21-34](file://src/components/archive-panel/ArchivePanel.vue#L21-L34)
...
- **新增** 样式依赖
  - 所有组件依赖 main.css 中的 Tailwind CSS v4 设计令牌。
  - AppLayout 通过 data-theme 属性控制主题模式切换。
...
- 主题切换
  ...
  - **新增** Tailwind CSS v4 使用 Lightning CSS 编译器，提供更快的样式处理性能。
- 面板宽度
  - 使用 store 集中管理宽度，配合最小/最大限制，避免极端尺寸导致的重排开销。
- **新增** 样式优化
  - Tailwind CSS v4 仅生成使用的原子类，显著减少 CSS 文件大小。
  - 设计令牌系统避免重复样式定义，提升样式复用效率。

## 故障排查指南
- 主题未生效
  - 确认 App.vue 已将 theme 与 themeOverrides 注入 NConfigProvider。
  - 检查 app.ts 的 isDarkTheme 是否正确切换。
  - **新增** 验证 AppLayout 的 data-theme 属性是否正确设置。
- 面板宽度异常
...
- **新增** Tailwind CSS 问题
  - 确认 vite.config.ts 中已正确配置 `@tailwindcss/vite` 插件。
  - 检查 main.css 中的 `@import "tailwindcss"` 指令是否正确。
  - 验证设计令牌变量是否在正确的 CSS 作用域内定义。
...
## 结论
Hello-Tauri 的界面系统以清晰的三栏布局为核心，结合组合式函数与 Pinia 状态管理，现已全面集成 Tailwind CSS v4 现代化样式系统，实现了主题切换、面板宽度调整、归档管理与多标签预览等功能。新的设计令牌架构提供了更好的可维护性和一致性，原子类的使用显著提升了开发效率和代码质量。组件职责明确、数据流向清晰，具备良好的可扩展性与可维护性。后续可在可访问性增强、更丰富的主题定制与更细粒度的性能监控方面持续优化。
...
### 样式定制与主题覆盖
- **新增** Tailwind CSS v4 设计令牌定制
  - 在 main.css 的 `@theme` 块中修改颜色、间距、圆角等设计令牌。
  - 支持通过 CSS 变量实现动态主题切换。
  - 使用 `color-mix()` 函数创建衍生颜色，提升主题一致性。
- 全局主题覆盖
...
- 自定义主题开发
  ...
  - **新增** 利用 Tailwind CSS v4 的 `@theme` 指令定义组件特定的设计令牌。
- 组件级样式
  - 优先使用 CSS 变量与 Tailwind 原子类；避免硬编码颜色。
  - 对于复杂动画和特殊效果，保留 scoped style 或使用 Vue 过渡系统。
...
### 测试环境增强
**新增** 测试环境现已完善 polyfill 支持：
- **拖拽事件支持**：实现 DataTransfer、DataTransferItem、DragEvent 的完整 polyfill
- **媒体查询支持**：mock window.matchMedia 方法，支持响应式组件测试
- **缓存存储模拟**：使用内存存储替代 IndexedDB，确保测试隔离性
- **Vitest 配置优化**：配置 jsdom 环境和全局设置
