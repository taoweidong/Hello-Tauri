...
**更新** 经过架构重构，PublicBar 现在专注于核心业务功能，移除了统计信息展示和主题切换功能，这些功能已迁移到 AppLayout 头部组件中。同时新增了外部链接功能，提升了用户体验。

## 项目结构
PublicBar 公共栏位于 src/components/public-bar 下，经过简化后包含两个核心子组件：
- PublicBar.vue：顶部工具栏容器，组合 GlobalSearch 与批量操作入口
...
- [GlobalSearch.vue:1-30](file://src/components/public-bar/GlobalSearch.vue#L1-L30)
...
## 核心组件
- PublicBar.vue
  - 职责：作为顶部公共栏容器，组织搜索区域和批量操作，提供简洁的工具栏界面。
  - 关键行为：通过 useArchiveManager 获取 archives 列表；处理"一键清空"等批量动作。
  - **更新** 完全移除了统计信息展示和主题切换功能，专注于核心工具。
- GlobalSearch.vue
...
## 架构总览
PublicBar 由两个子组件构成，分别负责布局和搜索。搜索流程从 UI 层到 Composable 再到核心服务，最终返回匹配结果与耗时指标。主题管理和外部链接功能已独立到 AppLayout 组件中。
...
### PublicBar 主容器
- 布局管理
  - 使用空间布局组件将搜索区与操作区分列两端，保持整体对齐与间距一致。
  - 高度与宽度占满父容器，便于嵌入应用头部。
- 响应式设计
...
- **更新** 职责大幅简化：移除了统计信息展示和主题切换功能，专注于核心的搜索操作和批量管理。
...
- AppLayout 中的外部链接功能
  ...
  - 统一的图标样式和悬停效果
- 职责分离优势
  - PublicBar 专注于业务功能（搜索、批量操作）
  - AppLayout 负责应用级配置（主题、时钟、外部链接、布局控制）
...
- 组件依赖
  ...
  - **更新** AppLayout.vue 现在负责主题管理、外部链接和时钟功能，不再依赖 PublicBar 中的相关功能。
- 核心服务依赖
...
- [use-search.ts:1-28](file://src/composables/use-search.ts#L1-L28)
- [search.ts:30-47](file://src/core/search.ts#L30-L47)
- [PublicBar.vue:14-18](file://src/components/public-bar/PublicBar.vue#L14-L18)
- [AppLayout.vue:60-95](file://src/layout/AppLayout.vue#L60-L95)

## 结论
PublicBar 公共栏经过架构重构后，以更加清晰的职责划分实现了搜索入口和批量操作的组合。通过移除统计信息展示和主题切换功能并将其迁移到 AppLayout 头部组件，进一步简化了公共栏的职责范围，提高了代码的可维护性。同时新增的外部链接功能提升了用户体验。当前搜索能力聚焦于接口调用与耗时统计，尚未集成结果展示与高亮跳转。通过引入索引化、防抖与并行化策略，可显著提升大规模文本检索的性能与用户体验。

**更新** 组件职责更加明确：PublicBar 专注于核心业务功能，AppLayout 负责应用级配置和外部链接，形成了更好的分层架构。
