...
## 更新摘要
**变更内容**   
- 更新了布局系统章节，详细说明新的 Flexbox 布局架构
- 增强了溢出处理和响应式设计的说明
- 添加了 NScrollbar 组件集成的技术细节
- 更新了渲染器容器的样式规范
- 完善了跨屏幕尺寸适配的实现方案
...
## 核心组件
- PreviewPane：负责加载当前活动标签页的文件内容，并根据解析结果动态渲染对应组件；使用错误边界包裹渲染区域，避免单个渲染器异常影响整体。**已重构为使用现代 Flexbox 布局**。
...
**更新** PreviewPane 组件已重构为使用现代 Flexbox 布局系统，提供更一致的跨屏幕尺寸行为。

#### Flexbox 布局架构
- **容器布局**：`.preview-container` 使用 `display: flex` 和 `flex-direction: column` 创建垂直弹性布局
- **滚动区域**：`.preview-scrollbar` 使用 `flex: 1` 自动填充剩余空间，`min-height: 0` 防止溢出问题
- **溢出控制**：容器设置 `overflow: hidden`，内部滚动条组件处理内容溢出

#### 响应式设计特性
- **自适应高度**：滚动区域根据父容器高度自动调整
- **内容适配**：支持不同屏幕尺寸下的内容显示
- **滚动优化**：使用 Naive UI 的 `NScrollbar` 组件提供统一的滚动体验

#### 状态管理
- **空状态**：无活动标签时显示"选择一个文件以预览"
- **加载状态**：文件加载中显示"加载中..."提示
- **渲染状态**：成功加载后显示对应的渲染器组件

**Section sources**
- [PreviewPane.vue:45-74](file://src/components/workspace/PreviewPane.vue#L45-L74)
- [Workspace.vue:39-52](file://src/components/workspace/Workspace.vue#L39-L52)
- [AppLayout.vue:440-446](file://src/layout/AppLayout.vue#L440-L446)
...
- PreviewPane 依赖：
  ...
  - 平台适配（adapter）
  - 错误边界（ErrorBoundary）
  - **新增**：Naive UI 滚动组件（NScrollbar）和空状态组件（NEmpty）
- ParserEngine 依赖：
...
PreviewPane --> NScrollbar : "滚动容器"
PreviewPane --> NEmpty : "空状态"
...
- **布局性能优化**
  - **新增**：Flexbox 布局相比传统布局方式具有更好的性能表现
  - **新增**：`min-height: 0` 解决了 Flexbox 子元素溢出问题
  - **新增**：NScrollbar 组件提供了优化的滚动性能
...
- 常见现象
  ...
  - **新增**：布局溢出问题：检查父容器是否正确设置了 `overflow: hidden` 和 `min-height: 0`
  - **新增**：滚动条不显示：确认 NScrollbar 组件是否正确包裹内容区域
- 定位步骤
  - 查看 ParserEngine.resolveFile 的返回值与耗时统计
  - 检查插件的 canParse 与 parse 逻辑是否符合预期
  - 在渲染器层增加空值与边界条件判断
  - **新增**：检查 Flexbox 布局的层级关系和 overflow 属性设置
- 恢复策略
  - 使用 ErrorBoundary 捕获渲染异常，回退到空态提示
  - 对解析失败的情况降级为十六进制或文本视图
  - **新增**：对于布局问题，检查父容器的 display: flex 和 flex-direction 设置
...
## 结论
PreviewPane 通过"解析器插件 + 渲染器组件"的解耦设计，实现了多格式文件的统一预览体验。**经过 Flexbox 布局重构后，组件在不同屏幕尺寸和内容类型下表现出更好的一致性行为**。当前版本在易用性与可扩展性方面表现良好，但在大文件渲染、搜索定位、语法高亮与缩放等方面仍有优化空间。建议优先引入虚拟滚动与按需高亮，以提升性能与交互体验。
...
- 渲染器组件约定：
  ...
  - 内部自行处理空值与边界情况
  - 建议使用等宽字体与暗色主题保持一致体验
  - **新增**：确保组件能够正确处理 Flexbox 布局环境下的溢出和滚动
...
- **新增**：确保渲染器组件在 Flexbox 环境中正确设置 `height: 100%` 和 `overflow: auto`
- **新增**：使用 `white-space: pre` 保持文本格式，配合 `font-family: monospace` 确保对齐
...
- 新建渲染器组件：
  - 接收 content 属性
  - 实现基础样式与交互
  - **新增**：确保组件适应 Flexbox 布局，正确处理溢出和滚动
- 在 PreviewPane 中自动生效：
  - 通过扩展名匹配到插件
  - 根据解析结果类型选择渲染器
