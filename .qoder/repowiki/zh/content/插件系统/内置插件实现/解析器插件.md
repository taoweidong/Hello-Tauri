...
## 更新摘要
**变更内容**   
- CSV解析器完全重写 - 实现符合RFC 4180规范的字段解析算法
- 新增parseLine函数，正确处理引号字段包含分隔符、转义引号和边界情况
- 增强CSV数据完整性保证，确保字段数量与表头一致
- 改进错误处理和空值处理机制
...
- [src/plugins/parser/csv-plugin.ts:1-31](file://src/plugins/parser/csv-plugin.ts#L1-L31)
- [src/plugins/parsers/csv-parser.ts:1-66](file://src/plugins/parsers/csv-parser.ts#L1-L66)
...
### CSV 解析器
**已更新** CSV解析器已完全重写，实现了符合RFC 4180规范的完整字段解析算法。

- 功能要点
  - **符合规范的字段解析**：实现完整的CSV规范解析，支持引号包裹的字段、转义引号、包含分隔符的字段值。
  - **parseLine函数**：新增独立的单行解析函数，确保每行字段的完整性和正确性。
  - **引号处理**：正确处理双引号转义（""表示单个引号）、引号内包含分隔符和换行符的情况。
  - **自定义分隔符**：支持逗号、制表符等多种分隔符，适配TSV等多格式。
  - **空行过滤**：自动过滤空行，提升鲁棒性。
  - **字段完整性保证**：确保每行字段数量与表头保持一致。
- 类型推断
...
- 性能特征
  - 基于状态机的字符级解析，时间复杂度 O(n)，空间复杂度取决于行数与列数；适合中小规模数据集。
- 错误处理模式
  - 空文本返回空结构，避免上层出现未定义访问；字段解析失败时保持数据完整性。
- 最佳实践
  - 新实现完全符合CSV规范，可处理复杂CSV文件；建议对含特殊字符的数据使用引号包裹。
- 使用场景
  - 数据报表、导出表格、批量导入前的预览与校验；特别适用于包含特殊字符的复杂CSV数据。
- 代码示例路径
  - [解析函数实现:1-66](file://src/plugins/parsers/csv-parser.ts#L1-L66)
  - [parseLine函数实现:10-48](file://src/plugins/parsers/csv-parser.ts#L10-L48)
  - [插件封装与配置模式:1-31](file://src/plugins/parser/csv-plugin.ts#L1-L31)
  - [单元测试用例:1-41](file://src/__tests__/plugins/parsers/csv-parser.test.ts#L1-L41)

```mermaid
flowchart TD
Start(["开始解析行"]) --> Init["初始化状态机<br/>current='', inQuotes=false"]
Init --> Loop{"遍历每个字符"}
Loop --> CheckQuotes{"是否在引号内?"}
CheckQuotes --> |是| QuoteChk{"是否为引号?"}
QuoteChk --> |是| EscapeChk{"下一个字符也是引号?"}
EscapeChk --> |是| AddEscaped["添加转义引号<br/>跳过两个字符"]
EscapeChk --> |否| ExitQuotes["退出引号模式"]
QuoteChk --> |否| AddChar["添加字符到current"]
CheckQuotes --> |否| DelimChk{"是否为分隔符?"}
DelimChk --> |是| PushField["推送字段到数组<br/>重置current"]
DelimChk --> |否| StartQuotes{"是否为引号?"}
StartQuotes --> |是| EnterQuotes["进入引号模式"]
StartQuotes --> |否| AddChar
AddEscaped --> Loop
ExitQuotes --> Loop
AddChar --> Loop
PushField --> Loop
EnterQuotes --> Loop
Loop --> |完成| Finalize["添加最后一个字段<br/>trim所有字段"]
Finalize --> End(["返回字段数组"])
```

图表来源
- [src/plugins/parsers/csv-parser.ts:10-48](file://src/plugins/parsers/csv-parser.ts#L10-L48)

章节来源
- [src/plugins/parsers/csv-parser.ts:1-66](file://src/plugins/parsers/csv-parser.ts#L1-L66)
- [src/plugins/parser/csv-plugin.ts:1-31](file://src/plugins/parser/csv-plugin.ts#L1-L31)
- [src/__tests__/plugins/parsers/csv-parser.test.ts:1-41](file://src/__tests__/plugins/parsers/csv-parser.test.ts#L1-L41)
...
- CSV 解析器
  - **已更新** 新的状态机解析算法虽然增加了字符级处理的开销，但提供了更高的准确性和规范性；对大数据集建议引入分块读取与增量解析，并在 UI 侧使用虚拟滚动。
- 日志解析器
...
- CSV 解析异常
  - **已更新** 现象：字段解析不正确、引号处理异常、分隔符识别错误。
  - 排查：确认分隔符配置；检查引号转义是否正确；验证parseLine函数的状态机逻辑；参考测试用例中的边界情况。
  - 参考路径：[parseLine函数实现:10-48](file://src/plugins/parsers/csv-parser.ts#L10-L48)、[解析函数实现:57-65](file://src/plugins/parsers/csv-parser.ts#L57-L65)、[相关测试:1-41](file://src/__tests__/plugins/parsers/csv-parser.test.ts#L1-L41)
- 日志解析不匹配
  - 现象：行级别为 OTHER、时间戳为空。
...
- [src/__tests__/plugins/parsers/json-parser.test.ts:1-41](file://src/__tests__/plugins/parsers/json-parser.test.ts#L1-L41)
- [src/plugins/parsers/csv-parser.ts:10-48](file://src/plugins/parsers/csv-parser.ts#L10-L48)
- [src/plugins/parsers/csv-parser.ts:57-65](file://src/plugins/parsers/csv-parser.ts#L57-L65)
- [src/__tests__/plugins/parsers/csv-parser.test.ts:1-41](file://src/__tests__/plugins/parsers/csv-parser.test.ts#L1-L41)
- [src/plugins/parsers/log-parser.ts:1-37](file://src/plugins/parsers/log-parser.ts#L1-L37)
- [src/__tests__/plugins/parsers/log-parser.test.ts:1-58](file://src/__tests__/plugins/parsers/log-parser.test.ts#L1-L58)
- [src/plugins/parser/hex-plugin.ts:1-53](file://src/plugins/parser/hex-plugin.ts#L1-L53)
...
## 结论
Hello-Tauri 的解析器插件体系以清晰的接口契约与模块化实现，提供了 JSON、CSV、日志、文本与十六进制查看器的完整能力。**特别是CSV解析器的完全重写，现在能够正确处理复杂的CSV文件格式，包括引号字段、转义字符和边界情况**。各解析器在错误恢复、类型安全与渲染集成方面表现稳健。针对大文件与复杂格式，建议在插件层引入分块读取、虚拟滚动与搜索节流等优化策略，进一步提升用户体验与系统性能。
...
- 使用场景建议
  ...
  - **已更新** CSV：报表与导出数据预览；新实现完全符合CSV规范，可处理包含特殊字符、引号和转义的复杂数据。
