...
## 简介
本文件为 PreviewToolbar.vue 预览工具栏组件的权威文档。该组件提供文本类预览的统一控制入口，包括字号调整、文本换行开关、行号显示控制与编码格式选择。它通过 v-model 双向绑定将状态上抛至父容器 Workspace.vue，由父容器统一维护并传递给渲染器，从而保证工具栏与预览面板的状态同步。

## 项目结构
预览工具栏位于工作区组件树中，处于标签栏下方、预览面板上方，负责暴露可配置的预览行为。其状态由父组件集中管理，并通过 props 下发给具体渲染器。

```mermaid
graph TB
A["Workspace.vue<br/>状态持有者"] --> B["PreviewToolbar.vue<br/>工具栏控件"]
...
## 架构总览
工具栏与预览面板之间的数据流遵循“父控子”的模式：父组件持有状态，子组件通过 v-model 上报变更，父组件再决定如何影响渲染器。
...
## 结论
PreviewToolbar.vue 提供了简洁直观的预览控制能力，并通过 v-model 与父组件保持状态同步。当前版本尚未将工具栏状态直接传递给渲染器，建议在父组件中统一分发，并在渲染器中消费这些状态以实现完整的可视化控制闭环。后续可扩展快捷键、无障碍与国际化能力，提升整体可用性与可维护性。
