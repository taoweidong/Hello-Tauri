...
**更新** 新增了标签页数量限制、高级上下文菜单操作和溢出处理等增强功能。
...
- **新增** FIFO 限制：当标签页数量超过 MAX_TABS（10个）时，自动淘汰最早的非固定标签页。
...
CheckLimit --> |是| Evict["淘汰最早的非固定标签页"]
Evict --> UpdateActive{"淘汰的是否为活动标签"}
UpdateActive --> |是| SwitchActive["切换到最后一个标签"]
SwitchActive --> RecordRecent["记录最近文件"]
UpdateActive --> |否| RecordRecent
RecordRecent --> End
...
- [use-tabs.ts:93-96](file://src/composables/use-tabs.ts#L93-L96)
- [use-tabs.ts:102-114](file://src/composables/use-tabs.ts#L102-L114)
- [use-tabs.ts:117-120](file://src/composables/use-tabs.ts#L117-L120)
- [use-tabs.ts:132-138](file://src/composables/use-tabs.ts#L132-L138)
...
- [use-tabs.ts:102-104](file://src/composables/use-tabs.ts#L102-L104)
- [use-tabs.ts:117-120](file://src/composables/use-tabs.ts#L117-L120)
...
- [use-tabs.ts:137](file://src/composables/use-tabs.ts#L137)
...
- 问题：关闭活动标签后界面空白
  ...
  - 参考：[use-tabs.ts:76-78](file://src/composables/use-tabs.ts#L76-L78)
- 问题：预览面板未渲染
...
- 问题：置顶标签无法关闭
  ...
  - 参考：[use-tabs.ts:93-96](file://src/composables/use-tabs.ts#L93-L96)
- **新增** 问题：标签页超过10个后无法正常打开新标签
  - 排查：检查 FIFO 淘汰逻辑是否正确执行，确认是否有固定标签阻止了淘汰。
  - 参考：[use-tabs.ts:51-61](file://src/composables/use-tabs.ts#L51-L61)
- **新增** 问题：右键菜单操作无效
  - 排查：确认 contextMenuTabId 是否正确设置，handleContextMenuAction 是否正确调用相应方法。
  - 参考：[TabBar.vue:48-74](file://src/components/workspace/TabBar.vue#L48-L74)

**更新** 新增了与 FIFO 限制和右键菜单相关的故障排查指南。

章节来源
- [use-tabs.ts:34-40](file://src/composables/use-tabs.ts#L34-L40)
- [use-tabs.ts:76-78](file://src/composables/use-tabs.ts#L76-L78)
...
## 结论
useTabManager 提供了简洁高效的标签页管理能力，配合 TabBar 与 PreviewPane 实现了完整的"打开—激活—渲染—关闭"闭环。当前实现聚焦于基础功能与内容懒加载缓存，现已增强 FIFO 限制、固定标签保护、右键菜单操作和溢出处理等功能。建议在后续迭代中补充历史记录、键盘导航与持久化机制，以进一步提升可用性与健壮性。

**更新** 强调了新增的增强功能及其对系统整体能力的提升。
