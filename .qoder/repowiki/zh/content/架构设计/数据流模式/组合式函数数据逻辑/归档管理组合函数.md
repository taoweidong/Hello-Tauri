...
图表来源
- [src/composables/use-archives.ts:1-59](file://src/composables/use-archives.ts#L1-L59)
...
## 核心组件
- useArchiveManager：维护全局归档列表、提供 addFiles/remove/updateStatus/stats/reset 等方法。
...
图表来源
- [src/composables/use-archives.ts:8-29](file://src/composables/use-archives.ts#L8-L29)
...
- 输入：File[]
- 处理：
  - 为每个 File 生成唯一 id（前缀 + 自增计数器），设置初始 status 为 pending，progress 为 0，files 为空数组，originalSize 为 0，compressedSize 取 file.size。
  - 将所有归档项推入内部列表。
...
### 删除与重置
- remove(id)：按 id 过滤移除对应归档项。
- reset()：清空 archives 并重置自增计数器，用于测试或场景切换。
