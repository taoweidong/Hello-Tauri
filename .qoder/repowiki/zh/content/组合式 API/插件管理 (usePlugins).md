...
- IFileParserPlugin：
  ...
  - parse：异步解析二进制数据，返回 ParsedResult
...
- 公共类型：
  - FileEntry、DecompressResult、ParsedContent 等
...
Decode --> BuildResult["构造 ParsedResult"]
...
- 统一输入输出：
  ...
  - 解析器输出：ParsedResult（type/data/lineCount）
  - 压缩器输入：Uint8Array 与输出目录
  - 压缩器输出：DecompressResult（success/files/error）
- 渲染契约：
  - 解析器通过 getComponent 返回渲染组件，由上层根据 ParsedResult.type 选择渲染器。
- 配置契约：
...
- 类型与配置：
  - 遵循 ParsedResult 与 DecompressResult 结构。
