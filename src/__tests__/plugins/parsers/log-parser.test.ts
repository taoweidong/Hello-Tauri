...
describe('parseLog', () => {
  it('解析正常日志行', () => {
    ...
    const lines = result.data as LogLine[]
  ...
  it('不匹配行归 OTHER 并保留 raw', () => {
    ...
    const result = parseLog(data)
    const lines = result.data as LogLine[]
    expect(lines[0].level).toBe('OTHER')
    expect(lines[0].timestamp).toBe('')
    expect(lines[0].module).toBe('')
    expect(lines[0].raw).toBe('这不是日志格式')
  })
  ...
  it('混合行（正常 + 异常）', () => {
    ...
    expect(lines[1].level).toBe('OTHER')
  ...
  it('模块名含连字符', () => {
    ...
    const result = parseLog(data)
    const lines = result.data as LogLine[]
    expect(lines[0].module).toBe('user-service')
  })
})
